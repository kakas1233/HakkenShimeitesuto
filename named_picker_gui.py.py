...
def run_app():
    st.title("🎲 年間指名表作成アプリ（最終改良版）")

    k = st.number_input("年間授業回数 k", value=30, min_value=1)
    l = st.number_input("1回の指名人数 l", value=5, min_value=1)
    n = st.number_input("クラス人数 n", value=40, min_value=1)

    name_input = st.text_area("✍️ 名前を改行区切りで入力（空欄なら自動生成）", height=150)
    ...
    if st.button("📊 指名表を作成する"):
        ...
        st.success(f"✅ 最小分散手法: {best_method}（seed={best_seed}, 分散={best_variance:.4f}）")
        st.info("🎯 指名ボタンを押すと1人ずつランダムに指名されます（重複なし）")
    ...
    if "named_pool" in st.session_state and "names" in st.session_state:
        ...
        if st.button("🎯 指名！"):
            ...
            if remaining_before:
                ...
                st.markdown(
                    f"""
                    <div style="font-size: 64px; font-weight: bold; text-align: center; color: #4CAF50; margin-top: 30px;">
                        🎉 {selected + 1}番：{names[selected]} 🎉
                    </div>
                    """,
