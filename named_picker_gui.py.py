...
# 変更前のコードは省略（そのままでOK）
# 以下をrun_app()の最後（if __name__ == "__main__": の直前）に追記
...
def permutation_entropy(time_series, order=3):
    ...
    for i in range(n - order + 1):
        sorted_idx = tuple(np.argsort(time_series[i:i + order]))
        patterns.append(sorted_idx)
    c = Counter(patterns)
    probs = np.array(list(c.values())) / sum(c.values())
    return entropy(probs)

def autocorrelation_score(series):
    """自己相関スコア（最大絶対相関, 低いほどランダム）"""
    ...
    if var == 0:
        return 0
    return max(abs(np.corrcoef(series[:-lag], series[lag:])[0, 1]) for lag in range(1, min(10, n)))

# このブロックを run_app() の最後の方に追加してください
history = st.session_state.get(tab + "_used", [])
if len(history) >= 5:
    pe = permutation_entropy(history)
    ac = autocorrelation_score(history)

    # 正規化（経験的に定めた目安）
    pe_norm = pe / np.log(math.factorial(3))  # order=3なので最大値 log(3!) ≈ 1.792
    ac_norm = 1 - ac  # 高いほどランダム性があると見なす

    st.markdown("### 🎲 ランダム性評価（直近の指名順）")
    st.markdown(f"""
    - 順列エントロピー: **{pe:.3f}**（正規化: {pe_norm:.3f}）※高いほどランダム
    - 自己相関スコア: **{ac:.3f}**（正規化: {ac_norm:.3f}）※低いほどランダム
    """)
