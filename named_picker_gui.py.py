...
def permutation_entropy(time_series, order=3, delay=1):
    ...
    for i in range(n - delay * (order - 1)):
        ...
        sorted_index = tuple(np.argsort(window))
        permutations[sorted_index] = permutations.get(sorted_index, 0) + 1
    total = sum(permutations.values())
    probs = np.array(list(permutations.values())) / total
    pe = -np.sum(probs * np.log2(probs))
    return pe / np.log2(factorial)

def autocorrelation_score(sequence):
    import numpy as np
